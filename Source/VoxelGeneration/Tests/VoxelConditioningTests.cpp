...
// ---------------------------------------------------------------------------
// Conditioning also flattens the other heightmap mode (IslandBowl)
// ---------------------------------------------------------------------------
IMPLEMENT_SIMPLE_AUTOMATION_TEST(FVoxelConditioningIslandBowlTest,
	"VoxelWorlds.Generation.Conditioning.FlattensIslandBowl",
	EAutomationTestFlags::EditorContext | EAutomationTestFlags::EngineFilter)

bool FVoxelConditioningIslandBowlTest::RunTest(const FString& Parameters)
{
	FVoxelCPUNoiseGenerator Generator;
	Generator.Initialize();
	...
	// A large circular island centered at the origin so the test chunk sits deep inside it (falloff ~1,
	// i.e. full terrain) and the surface lands inside the chunk's Z range.
	FVoxelNoiseGenerationRequest Request;
	Request.ChunkCoord = FIntVector(0, 0, 0);
	Request.ChunkSize = CS;
	Request.VoxelSize = VoxelSize;
	...
	Request.bEnableBiomes = false;
	Request.bEnableCaves = false;
	Request.IslandParams.Shape = 0;            // circular
	Request.IslandParams.IslandRadius = 200000.0f;
	Request.IslandParams.CenterX = 0.0f;
	Request.IslandParams.CenterY = 0.0f;
	Request.IslandParams.EdgeHeight = 0.0f;
	Request.IslandParams.bBowlShape = false;
	Request.NoiseParams.NoiseType = EVoxelNoiseType::Perlin;
	Request.NoiseParams.Seed = 909;
	Request.NoiseParams.Frequency = 0.013f;
	Request.NoiseParams.Amplitude = 1.0f;
	Request.NoiseParams.Octaves = 4;
	Request.NoiseParams.Lacunarity = 2.0f;
	...
	TArray<FVoxelData> Natural;
	TestTrue(TEXT("IslandBowl baseline generation succeeds"), Generator.GenerateChunkCPU(Request, Natural));
	const float NaturalInside = FindColumnSurfaceZ(Natural, CS, VoxelSize, 8, 8);
	const float NaturalOutside = FindColumnSurfaceZ(Natural, CS, VoxelSize, 0, 0);
	TestTrue(TEXT("IslandBowl baseline has a surface inside the chunk"), NaturalInside > 0.0f);

	const float TargetHeight = 1200.0f;
	Request.ConditioningZones.Add(FVoxelConditioningZone(FVector2D(800.0, 800.0), 300.0f, 100.0f, TargetHeight, 1.0f));

	TArray<FVoxelData> Conditioned;
	TestTrue(TEXT("IslandBowl conditioned generation succeeds"), Generator.GenerateChunkCPU(Request, Conditioned));
	const float CondInside = FindColumnSurfaceZ(Conditioned, CS, VoxelSize, 8, 8);
	const float CondOutside = FindColumnSurfaceZ(Conditioned, CS, VoxelSize, 0, 0);

	AddInfo(FString::Printf(TEXT("IslandBowl inside: %.0f -> %.0f (target %.0f); outside: %.0f -> %.0f"),
		NaturalInside, CondInside, TargetHeight, NaturalOutside, CondOutside));

	TestTrue(TEXT("IslandBowl footprint surface reaches the target height"),
		FMath::Abs(CondInside - TargetHeight) <= VoxelSize);
	TestEqual(TEXT("IslandBowl terrain outside the footprint is unchanged"), CondOutside, NaturalOutside);
